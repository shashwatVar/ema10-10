...
func (p *Processor) StartPolling() {
	...
	for {
		...
		case <-p.stopChan:
			...
			return
		default:
			nextTick := p.timeUntilNextTenMinMark()
...
func (p *Processor) checkOptionTrigger(optionType string, observedOption *OptionObservation) (Candle, error) {
	...
	for _, emaData := range p.state.EmaData {
		if emaData.Strategy == "EMA10-10" && emaData.CustomSymbol == p.state.OptionExpiry+" "+strconv.Itoa(int(strikePrice))+" "+symbolOptionType {
...
func (p *Processor) checkForTriggerFormation(currentCandle Candle, ema10 float64) bool {
	...
	if (currentCandle.Close < ema10) && (midPt > currentCandle.Close) {
		return true
	}
