...
var FetchAllClients = func() ([]*Ema10_10ClientDetails, error) {
	var clients []*Ema10_10ClientDetails
	query := `
		SELECT 
	...
	defer rows.Close()
	for rows.Next() {
		var client Ema10_10ClientDetails
		err := rows.Scan(
			&client.ID,
			&client.ClientID,
			&client.FixLots,
...
var FetchIndexConfig = func() (*Config, error) {
	var config Config
	query := `SELECT index_security_id, index_exchange_segment, index_instrument, option_expiry FROM ema10_10_config WHERE id = 1`
	err := db.QueryRow(query).Scan(
...
var FetchAllEma10Data = func() ([]*OptionsEmaConfig, error) {
	...
	query := `SELECT strategy, custom_symbol, ema FROM options_ema_config WHERE strategy = 'EMA10-10'`
	rows, err := db.Query(query)
	if err != nil {
